...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/freelancers", authenticateToken, async (req: any, res) => {
    try {
  ...
  app.put("/api/freelancers/:userId", authenticateToken, async (req: any, res) => {
    try {
  ...
  app.get("/api/users/:userId/projects", authenticateToken, async (req: any, res) => {
    try {
      const userId = parseInt(req.params.userId);
      if (userId !== req.userId) {
        return res.status(403).json({ message: "Not authorized" });
  ...
  // Application routes
  app.get("/api/projects/:projectId/applications", authenticateToken, async (req: any, res) => {
    try {
  ...
  app.post("/api/projects/:projectId/applications", authenticateToken, async (req: any, res) => {
    try {
      const projectId = parseInt(req.params.projectId);
  ...
  // Message routes
  app.get("/api/users/:userId/conversations", authenticateToken, async (req: any, res) => {
    try {
      const userId = parseInt(req.params.userId);
      if (userId !== req.userId) {
        return res.status(403).json({ message: "Not authorized" });
  ...
  app.get("/api/projects/:projectId/messages", authenticateToken, async (req: any, res) => {
    try {
      const projectId = parseInt(req.params.projectId);
      const project = await storage.getProject(projectId);
  ...
  app.post("/api/projects/:projectId/messages", authenticateToken, async (req: any, res) => {
    try {
      const projectId = parseInt(req.params.projectId);
